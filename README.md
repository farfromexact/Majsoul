...
If the page context does not expose `WebSocket` at the first document-start tick, the helper retries hook installation every 250 ms until it succeeds. The helper also passively retries read-only page decode hooks: `net.MessageWrapper.decodeMessage` when the Mahjong Soul client exposes it, and `Laya.EventDispatcher.event` when decoded game actions are dispatched through the page runtime. These hooks observe already-decoded visible fields without mutating messages or event payloads. The debug panel shows install attempts, whether `WebSocket` is currently available, whether client decode or page dispatch is hooked or still waiting, and the binary sample size used for capture exports. Increase `Binary sample bytes` before collecting a new capture if replay diagnostics report truncated raw binary samples.
...
The debug panel also includes a capture limit input. The default retained event buffer is 500 events, and changing it also changes the adapter's retained event buffer and is saved locally. Raise it up to 1000 before collecting a noisy real-page sample if the preflight reports dropped events. `Copy capture` and `Download capture` export only the most recent N helper events, which keeps real-page samples smaller and easier to share.
...
- Confirm Tampermonkey page injection captures live `raw_message` entries and that either the passive client-decode hook or the page-dispatch hook reaches `hooked` on the current production web client.
