...
Current `https://game.maj-soul.com/1/` builds can load as Unity WebGL instead of the older JS/Laya runtime. In that case the debug panel's `Runtime` line should show `Unity WebGL detected`; `client decode` and `page dispatch` may remain `waiting` even when WebSocket capture is healthy. Seeing `.lq.ActionPrototype` action names but no usable action payload fields means the helper has reached the passive raw layer, but state restoration still needs a Unity runtime hook or an action payload decoder.
