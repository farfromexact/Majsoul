...
Hook diagnostics include WebSocket constructor static-property copy counts and whether `prototype.constructor` was patched. This helps catch page compatibility issues where the game client expects constants such as `WebSocket.OPEN` or `socket.constructor === WebSocket`.
...
1. Install `majsoul-helper.user.js` in Tampermonkey.
