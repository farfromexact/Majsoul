...
2. Confirm the `Majsoul Helper v0.2.11` overlay appears. If the title does not show `v0.2.11`, Tampermonkey is still running an older generated userscript.
...
In `0.2.11` and later, own draw/discard traffic before a decoded initial hand is kept out of `gameState.hand`; this prevents replay from treating a few known self-drawn tiles as a complete base hand.
