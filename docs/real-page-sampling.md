...
2. Confirm the `Majsoul Helper v0.2.6` overlay appears. If the title does not show `v0.2.6`, Tampermonkey is still running an older generated userscript.
...
13. Leave `Capture limit` at the 500-event default for a first real sample, or raise it toward 3000 before collecting from the start of a noisy round. The helper also expands its retained in-page event buffer to match this value.
